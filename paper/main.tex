...
\title{Chronic Intestinal Failure in Brazil: A First Estimate of Prevalence}
...
This factor was held constant throughout the study period (2017--2025), as no more recent nationally representative estimates of the public-private split in hospital admissions were available. We acknowledge that this assumption may introduce some degree of bias if the relative utilization of public and private healthcare for CIF has changed over time, and we discuss this limitation accordingly.
...
As a cross-validation of the PN-based estimates, we conducted a complementary analysis restricted to hospital admissions recorded under the procedure code for intestinal failure rehabilitation at the hospital level (03.01.16.002-3), introduced by the Brazilian Ministry of Health in August 2024 through Portaria GM/MS N\textordmasculine~5.051/2024, as part of the new National Program for Comprehensive Care for Persons with Intestinal Failure (\textit{Programa Nacional de Aten\c{c}\~ao Integral \`{a}s Pessoas com Fal\^{e}ncia Intestinal}). Although the same ordinance also established an outpatient-level rehabilitation code (03.01.16.001-5), this was not included in the present analysis for two reasons. First, the SIH-SUS is structurally restricted to inpatient records and does not capture outpatient procedures. Second, given the severity of CIF in the pediatric population, virtually all cases in this age group require continuous hospital-level care, rendering the outpatient code unlikely to reflect meaningful additional case volume. This complementary analysis is necessarily limited to the period from August 2024 onward, when the hospital-level code became operational, and is therefore interpreted as a descriptive snapshot rather than a longitudinal estimate. Its primary value lies in providing an independent, diagnosis-specific reference point against which the PN-based prevalence estimates can be assessed.

\subsection*{Patient Profile Analysis}

For patients identified as probable CIF cases through the PN-based strategy, we conducted a descriptive characterization to document the clinical and demographic profile of this population, as well as patterns of healthcare utilization. Demographic variables included age at admission, sex, and race/ethnicity where recorded. The clinical profile was examined through the primary ICD-10 diagnosis registered at discharge and the co-registered procedures within the same admission, which together allow inference about the underlying conditions associated with CIF in the Brazilian pediatric population. To characterize the intensity of healthcare use, we reported the mean number of hospital admissions per patient, the mean duration of admissions in daily units (\textit{di\'{a}rias}), and the in-hospital mortality rate, the latter being particularly informative given that untreated or inadequately treated CIF carries substantial mortality risk \cite{mukhopadhyay2019}. Geographic patterns were assessed by mapping the state (\textit{Unidade Federativa}, UF) of patient residence alongside the location of the admitting hospital, and by estimating the travel distance between the centroid of each patient's postal code and the municipality of the treating hospital. This geographic dimension is important because the concentration of CIF treatment capacity in a small number of specialized centers may impose significant displacement burdens on families, with potential implications for access to and continuity of care.
...
The proportion of white patients in the CIF cohort exceeded their share in the general Brazilian population throughout most of the study period, ranging from approximately 42\% to 52\% (Figure~\ref{fig:raca}). This pattern must be read with caution. Race and ethnicity are substantially underreported in SIH-SUS records---the same incompleteness that led us to exclude the field from the linkage algorithm---so the observed distribution may reflect differential completeness of race recording across regions and facilities as much as any real difference in who reaches care. We therefore report the figure descriptively and do not treat it as established evidence of differential access; whether access to prolonged hospital-based parenteral nutrition varies by race is a question this data source cannot settle.
...
Taken together, these findings provide direct empirical support for the interpretation advanced throughout this results section: that the PN-based prevalence estimates reported here capture only the portion of the pediatric CIF population that successfully accessed prolonged parenteral nutrition within the SUS and whose care was recorded under a relevant procedure code. The low uptake of the CIF-specific code introduced in 2024, even in the context of an explicit national policy mandate, illustrates the broader challenge of generating reliable epidemiological data on rare conditions within large, heterogeneous administrative health systems, and underscores the likelihood that the true prevalence of pediatric CIF in Brazil is meaningfully higher than the figures reported here.

\section{Discussion}\label{sec4}

This study provides the first national, population-based estimate of pediatric CIF prevalence in Brazil. Over the 2017--2025 period, the mean annual prevalence was 3.35 cases per million children and adolescents aged 0 to 17 years, derived from prolonged parenteral nutrition recorded in nationwide hospital admission data. Given the identification strategy and the structure of the underlying administrative records, this figure is best read as a lower bound: it captures the subset of affected children who reached and were retained within specialized, publicly funded care long enough for chronic PN exposure to appear in the SIH-SUS. Several strands of evidence assembled here point in the same direction: the pandemic-era decline without full recovery, the concentration of treating hospitals in a few centers, the mortality rate above what adequate treatment achieves, and the near-absence of uptake of the CIF-specific procedure code. Together they indicate that the true burden is meaningfully higher than the recorded count.

Placed against the international literature, the estimate sits well below the rates reported for high-income settings, where pediatric SBS-associated intestinal failure has been estimated at roughly 14.9 per million in the United States and 16.7 per million in the United Kingdom \cite{khokhar2025global}. A comparable pattern of concentration is visible in UK pediatric home parenteral nutrition provision, which is both regionally uneven and increasingly dominated by SBS cases \cite{beath2011trends}. Relative to the high-income prevalence benchmarks, 3.35 per million is low by a factor of four to five. It would be a misreading, however, to interpret the Brazilian figure as anomalously small. The only prior country-specific estimate for Brazil, obtained by Khokhar and colleagues through modeled extrapolation, was on the order of 1.4 per million \cite{khokhar2025global}; the administrative-data approach adopted here recovers more than twice that number. The design therefore surfaces cases that naive extrapolation misses, while still falling far short of high-income rates---a pattern consistent with underdiagnosis and incomplete case ascertainment rather than a genuinely lower incidence of a condition whose etiology is largely congenital and neonatal.

The most direct point of comparison comes from a recent United States claims-database analysis, which identified pediatric SBS-associated intestinal failure at approximately 9 per million using a PN-claims proxy with patient-level deduplication \cite{ali2025epidemiology}. This estimate is instructive on two counts. First, it is the closest methodological analogue to the present study, and it independently confirms that a PN-based proxy applied to routinely collected billing records is a legitimate route to CIF ascertainment. Second, that a claims-based approach in the United States returns a figure well below the survey- and registry-based US benchmark reinforces the reading of our own result: administrative case-finding recovers a floor, not a ceiling. The remaining gap between the Brazilian and US claims-based figures is consistent with the access barriers documented throughout the Results.

The methodological choices made here have direct precedent. In the absence of a unique diagnostic code, procedure-based proxies have been used to study SBS in large administrative systems: analyses of the US Kids' Inpatient Database identified cases through procedure codes precisely because SBS lacks a dedicated identifier, albeit at the discharge rather than the patient level \cite{pant2017epidemiology}. Our patient-level linkage addresses that discharge-versus-patient limitation. Deterministic matching on fields such as date of birth, municipality, and postal code has been applied to Brazilian de-identified administrative health records in other settings \cite{medinacoeli2025deterministic}; we adapt that general approach to the pediatric CIF context, and its accuracy in this specific population, particularly the risk of merging multiple-birth siblings who share a postal code and date of birth, is a limitation we return to below. The PN proxy and the linkage strategy thus build on methods used elsewhere in administrative-data research, applied here to a population in which their error properties have not previously been characterized.

The middle-income context further frames the interpretation. The RESTORE registry, spanning twenty Latin American centers, documents a pediatric SBS-associated intestinal failure population whose leading etiologies---intestinal atresia, necrotizing enterocolitis, and gastroschisis---closely mirror the neonatal-morbidity profile observed in our data \cite{rumbo2024restore}. Within Brazil, a recent multicenter study of 207 patients across three centers in São Paulo and Rio Grande do Sul characterized intestinal rehabilitation in the country as constrained by limited financial resources, insufficient access to the few reference centers, inadequate multidisciplinary training, diagnostic delay driven by low disease awareness, and structural obstacles to home parenteral nutrition \cite{leite2025multicenter}. That study reported no general population figures for Brazil, which is the precise void this analysis addresses.

The trajectory of the estimates over time reinforces the access-barrier interpretation. The sharp decline in recorded cases during 2020 and 2021 is implausible as a fall in true incidence, since CIF arises predominantly from congenital and neonatal conditions unaffected by a pandemic over such a horizon. It is far more consistent with the documented disruption of pediatric CIF care during COVID-19, when constrained hospital capacity curtailed access to the prolonged parenteral nutrition on which case identification depends \cite{hilberath2024impact}. The failure of case counts to return to pre-pandemic levels in subsequent years is compatible with a portion of that cohort not surviving to be counted. We advance this hidden-mortality interpretation as a hypothesis rather than a finding: the administrative record cannot observe deaths outside the hospital system, and our data cannot distinguish non-survival from other explanations, such as a lasting contraction of access or shifts in coding practice.

The geographic findings expose a second structural problem. Treating hospitals were concentrated in a small number of South and Southeast centers, more than half of identified patients traveled outside their municipality of residence, and mean travel distance approached 70 km. The cross-validation using the 2024 CIF-specific code sharpened this picture: the two institutions accounting for essentially all coded admissions include the Hospital de Clínicas de Porto Alegre, which operates Brazil's most established pediatric intestinal-rehabilitation program \cite{goldani2022outcomes}. That a single mature program dominates the diagnosis-specific record is less a statement about where disease occurs than about where the infrastructure to diagnose, treat, and correctly code it exists. The near-total absence of identified cases in the North, Northeast, and Central-West points to unmet need concentrated where specialized capacity is thinnest.

The etiological profile warrants a careful contrast. In our data, pediatric CIF presented overwhelmingly as a complication of prematurity and its associated neonatal morbidities rather than of congenital enteropathies or motility disorders. This distinction is real but should not be overstated. Contemporary surgical cohorts in high-income settings are themselves NEC-dominated: a Nordic multicenter series of pediatric SBS, for instance, was driven largely by necrotizing enterocolitis \cite{mutanen2023nordic}. The contrast with a congenital-enteropathy profile is therefore truer of cross-sectional PN-center registries than of surgical SBS cohorts, and the Brazilian pattern is broadly consistent with international outcome benchmarks for the condition \cite{avitzur2024iifr}.

Finally, the low uptake of the CIF-specific code echoes a broader difficulty in generating reliable epidemiological data on rare conditions in Brazil. The recent establishment of RARAS, the country's first national rare-disease registry, marks initial progress toward systematic case capture \cite{andrade2025paving}, but the results here show how far routine administrative coding remains from that goal for CIF specifically.
...
This study provides the first national, population-based estimate of pediatric Chronic Intestinal Failure prevalence in Brazil: a mean of 3.35 cases per million children and adolescents aged 0 to 17 years over 2017--2025, built from prolonged parenteral nutrition recorded in nationwide hospital admission data. The estimate should be read as a lower bound. The gap relative to high-income settings, the incomplete post-pandemic recovery, the concentration of care in a handful of centers, and the minimal uptake of the CIF-specific procedure code together point to underdiagnosis, underreporting, and unequal access rather than to a genuinely lower incidence of the condition.

These findings identify concrete priorities for policy. Standardizing and diffusing the CIF-specific procedure code across hospitals would allow routine administrative data to track the condition rather than obscure it. Prospective clinical registries---building on the national rare-disease infrastructure now emerging in Brazil---would supply the anatomical, nutritional, and follow-up detail that billing records cannot. Coordinated referral networks and the expansion of specialized pediatric parenteral-nutrition capacity beyond the South and Southeast would reduce the geographic inequities that currently require families of critically ill infants to travel long distances for life-sustaining care. As a descriptive estimate, this work cannot establish the causes of the measured burden; what it does establish is that the burden of pediatric CIF in Brazil is real, unevenly borne, and larger than the recorded figures suggest. Closing that gap begins with making these children visible to the health system that must care for them.
...
% Appendix scaffold — populate before submission with, e.g., the STROBE
% participant-flow diagram, the ICD-10 code appendix for the diagnosis/procedure
% tables, and the threshold/scaling sensitivity analyses requested in review.
% Commented out so the empty placeholder does not render.
% \newpage
% \begin{appendices}
